...
- A review of an unpublished implementation commit must reuse the implementation issue's execution workspace. Set `inheritExecutionWorkspaceFromIssueId` to the implementation issue, `executionWorkspacePreference` to `reuse_existing`, and add the implementation issue as the review's sole `blockedByIssueIds` entry. `reuse_existing` without an explicit source or workspace id is invalid: it can silently create a fresh `origin/main` worktree that does not contain the commit under review. Before the review wake, read back that the review issue points to the implementation execution workspace and expected commit; otherwise fail closed.
